...
$layout = new TLayout("export", "Purific1GreenWave1", "MobileGreenWave1");
$layout->version = 2;
...
$page_layouts["school_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["school_id"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["school_id"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
